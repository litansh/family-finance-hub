...
- המשפחה יכולה "להתחייב" לסכום חודשי בקטגוריה משתנה שבפועל היא קבועה (סופר, פארמה). ב־get_overview קטגוריה כזו מסומנת committed_by_the_family, הסכום שהתחייבו אליו הוא התקציב שלה, ו־after_fixed_and_commitments מראה מה נשאר לכל שאר ההוצאות המשתנות אחרי הקבועות וההתחייבויות. כששואלים "כמה נשאר לי" — ענה לפי זה.
