...
- לכל רובריקה במעקב (סופר, פנאי, בריאות וכו') יש יעד חודשי שהמשפחה קבעה ברייזאפ; זה ה־budget שלה ב־get_overview. אם המשפחה התחייבה במרכז הכספים לסכום אחר, הרובריקה מסומנת committed_by_the_family והסכום שלה הוא התקציב (budget_in_riseup מראה את המקורי). after_fixed_and_rubrics מראה מה נשאר לכל שאר ההוצאות המשתנות אחרי הקבועות ואחרי היעדים של כל הרובריקות. כששואלים "כמה נשאר לי למשתנות" — ענה לפי זה, והבחן בין מה שנשאר בתוך רובריקה לבין מה שנשאר מחוץ לרובריקות.
