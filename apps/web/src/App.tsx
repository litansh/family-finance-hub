...
export function App() {
  ...
  const ctx: Ctx | undefined = useMemo(() => d && { d, theme, open: setDetail, openTxn: (t: ViewTransaction) => setDetail({ kind: 'txn', t }), setReco, go, saveStrategy: (s) => { saveStrategy(s).catch(() => say('ההנחות נשמרו במכשיר הזה בלבד')); }, setCommitment: async (label, amount) => { try { await saveCommitment(label, amount); await load(); say(amount === null ? 'ההתחייבות הוסרה' : 'נשמר. זה התקציב של הקטגוריה מעכשיו'); } catch (e) { say((e as Error).message); } }, undoShift: async (s) => { try { await saveShift({ month: s.month, from: s.to, to: s.from, amount: s.amount, reason: 'ביטול העברה' }); await load(); say('ההעברה בוטלה'); } catch (e) { say((e as Error).message); } }, savePlans: (plans) => { savePlans(plans).catch(() => say('התוכנית נשמרה במכשיר הזה בלבד')); } }, [d, theme]); // eslint-disable-line react-hooks/exhaustive-deps
